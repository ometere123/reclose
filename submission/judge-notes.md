...
Please distinguish source/test claims from live-chain claims. The active Studio-dev policy and its bindings are evidenced in `deployment/61997/r1-lifecycle-split-run-a-working-manifest.json`. The canonical live incident scenario was intentionally not submitted after a minimal accepted-message reproduction failed with `SystemError: 2: inval`; the finalized control succeeds. E1 A+B and H1 are therefore incomplete. No claim of successful live containment, recovery, benchmark rates or external audit PASS is made.
