...
const MANIFEST = "deployment/61997/r1-final-generation-manifest.json";
...
let manifest = await exists(MANIFEST) ? JSON.parse(await read(MANIFEST)) : {
  ...
  targetId: TARGET_ID, deployer: account.address, contracts: {}, wiring: {}, policy: {}, createdAt: new Date().toISOString()
};
...
const registryHash = "0x30148f85dc4c0dab65b7d1db38c863160d98f1f4f4f72f3d67dc5e287918f65a";
const registryJson = await read("config/source-registry-e1a.json");
const judge = await deploy("IncidentJudgeV1", "contracts/incident_judge_v1.py", [kernel, 1, registryHash, registryJson]);
...
if (String(reads.targetId) !== TARGET_ID || String(reads.targetOwner).toLowerCase() !== account.address.toLowerCase() || String(reads.targetController).toLowerCase() !== kernel.toLowerCase() || Number(reads.targetState) !== 0 || reads.targetRevoked !== false || String(reads.judgeKernel).toLowerCase() !== kernel.toLowerCase() || String(reads.judgeVault).toLowerCase() !== vault.toLowerCase() || Number(reads.judgeVersion) !== 1 || String(reads.judgeRegistry).toLowerCase() !== registryHash.toLowerCase() || String(reads.vaultKernel).toLowerCase() !== kernel.toLowerCase() || String(reads.vaultJudge).toLowerCase() !== judge.toLowerCase()) throw new Error(`Binding readback failed: ${JSON.stringify(reads)}`);
manifest.readbacks = reads; manifest.registry = { path: "config/source-registry-e1a.json", hash: registryHash }; manifest.lifecycle = "VERIFIED"; await save();
