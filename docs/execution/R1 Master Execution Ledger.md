...
| 67-71 | E1/H1 | canonical live scenario x2, 78-scenario benchmark, hardening | BLOCKED / NOT RUN - lifecycle-specific Kernel/Judge stack and policy-r1-009 are active; treasury balance readback is 0.20 GEN. Explicit accepted Kernel simulation returns `SystemError: 2: inval`; independent accepted-vs-finalized Parent→Child noop reproduction confirms the current Studio-dev limitation. No incident submitted. | release-evidence/r1/e1/fixtures/; release-evidence/r1/diagnostics/accepted-message-repro/; deployment/61997/r1-lifecycle-split-run-a-working-manifest.json | scripts/check-e1-evidence.mjs; scripts/r1-final-run-a-incident-prepare.mjs; scripts/run-studio-dev-accepted-message-repro.mjs | E1 A/B not run to completion; H1 not run | ac119d7 | Funding readback available; screenshot transaction hash truncated and not inferred. Studio-dev displayed v0.123.0-rc.6; exact backend SHA unexposed. Resume only when explicit accepted-message simulation succeeds. |
