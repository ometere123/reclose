...
| 67-71 | E1/H1 | canonical live scenario x2, 78-scenario benchmark, hardening | BLOCKED / NOT RUN - isolated typed-address `emit_decided` Parent→Child simulation succeeded read-only, but active immutable Kernel/Judge source predates the correction. No full Judge→Kernel→Target incident, containment, remediation, recovery, Run A/B or H1 metrics are verified. | release-evidence/r1/diagnostics/accepted-message-repro/typed-address-existing-parent/result.json; deployment/61997/r1-lifecycle-split-run-a-working-manifest.json | scripts/check-e1-evidence.mjs; scripts/r1-final-run-a-incident-prepare.mjs; benchmark/r1-scenarios.json | E1 A/B and H1 not run to completion; final live fee coverage incomplete | 67d30e5 | Source-matched immutable deployment, transaction-specific approvals, current-wallet browser proof and canonical readbacks remain required. |
