...
Current next step: wait for the user's wallet funding transaction and verify it. Then run `node scripts/r1-final-run-a-incident-prepare.mjs deployment/61997/r1-lifecycle-split-run-a-working-manifest.json`; proceed to the guarded incident writer only if the complete fee preset succeeds. Keep all Studio-dev requests serialized with the 2.6-second guard and backoff.
