Current phase: **R1 submission closeout — live-default frontend and Transaction Kit integration are implemented locally; public deployment, real-wallet proof, source-matched chain stack, E1 Run A/B, H1 and final fee coverage remain incomplete.**

**Current OB-014 disposition (2026-09-14): isolated `decided` message-emission validation CLOSED.** The corrected one-time read-only simulation succeeded on chain 61997 against the existing Parent with typed `CalldataAddress` calldata and the saved accepted allocation. The result records `transactionSubmitted: false`; returned fee values are estimates, not charges. The immediately prior failure encoded Child as a string and failed before message emission, so it is not evidence against the phase. This verifies only the isolated emission path; full Reclose lifecycle, E1 Run A/B, H1, and final live fee coverage remain blocked/not run. See the saved result and hash in `docs/execution/OB-014 Root Cause Report.md`.

The production frontend entry now installs its own live SDK runtime by default; mock data is available only through the explicit `?mode=preview` route. The wallet review uses pinned `@genlayer/transaction-kit@0.1.0-rc.2` and its React adapter. Local signing remains fail-closed because the active immutable Kernel/Judge generation predates the `decided` correction. The public host is GitHub Pages at <https://ometere123.github.io/reclose/> (workflow deployment succeeded for the Pages commit); no real-wallet browser proof exists. The deployed overview loaded in Chrome as LIVE on studio-dev/61997 but remained at “Reading protocol state,” so this is not yet proof of canonical reads or writes. No full incident lifecycle, Run A/Run B artifacts, or final fee profile exists. Preserve A2-C01's final-only live-verified result. Historical phase entries below are chronology and are superseded where they conflict with this current snapshot.
