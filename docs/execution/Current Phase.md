Current phase: **R1 release preparation — E1 Run A/B and H1 are blocked/not run; no incident write submitted.**

**Current OB-014 diagnosis (2026-09-14):** the retained minimal Parent→Child `noop()` reproduction uses one real SDK-estimated, explicitly encoded allocation. Its accepted emitter fails at the `EmitInternalMessage` `wasi.gl_call` boundary with `SystemError: 2: inval`; the otherwise matched finalized control succeeds. This excludes Reclose policy logic, repeated Target-message grouping, and the old Judge sibling call-key collision as causes of this minimal failure. Studio-dev UI reports `v0.123.0-rc.6`, but the hosted service does not expose an exact backend SHA or the validation trace needed to identify the rejecting component/rule. The environment-level accepted simulation limitation is reproduced; its inner server-side cause remains unresolved. Do not rerun the same simulation, submit an incident, or change fee values/phases speculatively. See `docs/execution/OB-014 Root Cause Report.md`, `docs/execution/Open Blockers.md` OB-014, and `release-evidence/r1/diagnostics/accepted-message-repro/`.

No current-wallet browser proof, complete live incident lifecycle, Run A/Run B artifacts, or final fee profile exists. Continue independent, non-live release work while keeping blocked/not-run evidence distinct from passes. Preserve A2-C01's final-only live-verified result. Historical phase entries below are chronology and are superseded where they conflict with this current snapshot.
