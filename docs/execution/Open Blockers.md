...
## OB-014 current disposition (2026-09-14; see root-cause report)

The accepted-message behavior is independently reproduced with a minimal disposable Parent and Child using a real estimator-produced explicit allocation. `on="accepted"` fails at the `EmitInternalMessage` `wasi.gl_call` boundary with `SystemError: 2: inval`; `on="finalized"` succeeds. Exact source, transaction evidence, and redacted structured responses are frozen under `release-evidence/r1/diagnostics/accepted-message-repro/`. This establishes an accepted-message simulation limitation in the exposed Studio-dev environment, independent of Reclose policy logic. It does **not** identify the specific server-side validation or component: Studio-dev reports version `v0.123.0-rc.6`, while exact backend SHA and the validation trace are unavailable. The source-tag fee rules are documented separately; exact source-to-deployed-build correspondence is not claimed.

No E1 incident or Reporter nonce exists for the current generation. Do not repeat the same simulation, submit an incident, or claim Run A/B or H1 pass. The precise missing evidence is a diagnostic for the existing captured simulation request that identifies the rejected host-call condition and, to attribute it to a component, its deployed build/source mapping. Continue independent documentation/traceability work. See `docs/execution/OB-014 Root Cause Report.md` for facts, inference, and release consequences.
