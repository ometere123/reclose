...
## OB-014 current disposition (2026-09-14)

The accepted-message behavior is independently reproduced with a minimal disposable Parent and Child using a real estimator-produced explicit allocation. `on="accepted"` fails at `wasi.gl_call` with `SystemError: 2: inval`; `on="finalized"` succeeds. Exact source, transaction evidence and redacted structured responses are frozen under `release-evidence/r1/diagnostics/accepted-message-repro/`. This is a current Studio-dev simulation limitation; deployed version tag is `v0.123.0-rc.6`, exact backend SHA is unavailable. No E1 incident was submitted. Do not run B or claim H1 pass until the canonical prerequisite becomes executable. Continue independent documentation/traceability work.
